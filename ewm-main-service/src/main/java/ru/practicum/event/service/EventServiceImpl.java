...
public class EventServiceImpl implements EventService {
    ...
    public Map<String, List<ParticipationRequestDto>> approveRequests(Long userId, Long eventId,
        ...
        boolean verified = requests.stream().allMatch(r -> r.getEvent().getId().longValue() == eventId);
        if (!verified) {
    ...
    public List<EventFullDto> getAllByAdmin(List<Long> users, List<String> states, List<Long> categories,
        ...
        if (users == null || users.isEmpty()) {
            ...
            userList = userRepository.findByIdInOrderByIdAsc(users, pageRequest);
        ...
        if (categories == null) {
            ...
            categoryList = categoryRepository.findByIdInOrderByIdAsc(categories, pageRequest);
        ...
        List<Event> events = eventRepository.findByInitiatorInAndStateInAndCategoryInAndEventDateAfterAndEventDateBeforeOrderByIdAsc(
                userList, eventStates, categoryList, start, end, pageRequest
        );
        if (events.isEmpty()) {
    ...
    public EventFullDto approveEventByAdmin(Long eventId, EventUpdateDto eventUpdateDto) {
        ...
        Optional.ofNullable(eventUpdateDto.getPaid()).ifPresent(oldEvent::setPaid);
        Optional.ofNullable(eventUpdateDto.getRequestModeration()).ifPresent(oldEvent::setRequestModeration);
        if (eventUpdateDto.getStateAction() != null && oldEvent.getState().equals(EventState.PENDING)
    ...
    public List<EventShortDto> getAllPublic(String text, List<Long> categories, Boolean paid,
        ...
        queryStr.append(" AND e.state = 'PUBLISHED'");
        ...
        if (onlyAvailable) {
            events.removeIf(e -> e.getParticipantLimit() != 0
                    && Objects.equals(e.getParticipantLimit(), e.getConfirmedRequests()));
        }
        ...
        List<Event> paginatedEvents = events.stream().skip(from % size).toList();
